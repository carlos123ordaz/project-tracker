...
import { Plus, Search, Trash2, X, Edit2, Check, Package, ShoppingCart, ImageIcon, Upload, Columns, ChevronLeft, ChevronRight, ChevronUp, ChevronDown } from 'lucide-react'
import { useComprasBOM } from '../../hooks/useComprasBOM'
import type { ProcesoBOM, EstadoCotBOM, ComprasBOMItemInsert, ComprasBOMItem, SortFieldBOM, SortDirBOM } from '../../hooks/useComprasBOM'
...
const PRICE_COL_MAP: Record<string, typeof PROVEEDORES[0]> = {
...
const PRICE_BG: Record<string, string> = {
  polimetales: '#fefce8', othero: '#f0fdf4', pernos: '#eff6ff', ducasse: '#fdf4ff', em_metal: '#fff7ed',
}
...
interface ColDef { key: ColKey; label: string; defaultVisible: boolean; group?: 'precios' }
...
const COL_VIS_KEY   = 'bom_col_visibility_v1'
const COL_ORDER_KEY = 'bom_col_order_v1'
const CUSTOM_KEY    = 'bom_custom_cols_v1'
...
const emptyForm = (): Omit<ComprasBOMItemInsert, 'sort_order'> => ({
  ...
  imagen: null, estado_cot: 'PENDIENTE', observaciones: '', custom_fields: {},
})
...
export default function SolicitudCompraBOMPage() {
  ...
  const [sortField, setSortField]         = useState<SortFieldBOM>('sort_order')
  const [sortDir, setSortDir]             = useState<SortDirBOM>('asc')
  const [page, setPage]                   = useState(1)
  const [pageSize, setPageSize]           = useState(15)

  const { items, total, estadoCounts, loading, error, create, update, remove } =
    useComprasBOM({ page, pageSize, search, estado: filterEstado, proceso: filterProceso, sortField, sortDir })
  ...
  const totalCols = 4 + orderedVisibleCols.length
  ...
  function openEdit(it: ComprasBOMItem) {
    setEditId(it.id)
    const { id, created_at, updated_at, sort_order, ...rest } = it
    setForm({ ...rest, custom_fields: it.custom_fields ?? {} })
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14, flexWrap: 'wrap' }}>
        ...
        <select value={filterProceso} onChange={e => { setFilterProceso(e.target.value as ProcesoBOM | 'TODOS'); setPage(1) }} style={selectS}>
          <option value="TODOS">Todo proceso</option>
          <option value="Compra">Compra</option>
          <option value="Fabricación">Fabricación</option>
        </select>

        <select value={sortField} onChange={e => { setSortField(e.target.value as SortFieldBOM); setPage(1) }} style={selectS}>
          <option value="sort_order">Orden personalizado</option>
          <option value="item">Ordenar por Ítem #</option>
          <option value="descripcion">Ordenar por Descripción</option>
          <option value="proceso">Ordenar por Proceso</option>
          <option value="estado_cot">Ordenar por Estado</option>
          <option value="created_at">Ordenar por Creación</option>
        </select>

        <select value={sortDir} onChange={e => { setSortDir(e.target.value as SortDirBOM); setPage(1) }} style={selectS}>
          ...
          <div ref={colPickerRef} style={{ position: 'relative' }}>
            ...
            {showColPicker && (
              <div style={{
                ...
                boxShadow: '0 8px 24px rgba(0,0,0,.10)', width: 272, padding: '10px 0',
              }}>
                <div style={{ padding: '0 12px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--n-100)' }}>
                  ...
                  <button onClick={resetCols} style={{ fontSize: 10.5, color: 'var(--brand-700)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: 0 }}>
                    Restaurar
                  </button>
                </div>

                {/* Lista de columnas en orden actual */}
                <div style={{ maxHeight: 300, overflowY: 'auto', padding: '4px 0' }}>
                  {colOrder.map((key, idx) => {
                    ...
                    if (!col) return null
                    const isPrice = isBuiltIn && (col as ColDef).group === 'precios'
                    return (
                      <div key={key}
                        ...
                        <span style={{ flex: 1, fontSize: 12.5, color: isPrice ? 'var(--n-500)' : 'var(--n-700)', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {col.label}
                          {!isBuiltIn && <span style={{ fontSize: 10, color: 'var(--brand-500)', marginLeft: 4 }}>•</span>}
                        </span>
                        <div style={{ display: 'flex', gap: 1, flexShrink: 0 }}>
                          <button onClick={() => moveCol(key, 'up')} disabled={idx === 0}
                            title="Subir"
                          ...
                          <button onClick={() => moveCol(key, 'down')} disabled={idx === colOrder.length - 1}
                            title="Bajar"
                ...
                <tr style={{ background: 'var(--n-50)', borderBottom: '1px solid var(--n-150)' }}>
                  ...
                  {orderedVisibleCols.map(key => {
                    if (BUILT_IN_KEY_SET.has(key)) {
                      const col = BUILT_IN_COL_MAP[key as ColKey]
                      const bg = PRICE_BG[key]
                      return <th key={key} style={bg ? { ...thS, background: bg } : thS}>{col.label}</th>
                    }
                    ...
                    <tr key={it.id} style={{ borderBottom: i < items.length - 1 ? '1px solid var(--n-100)' : undefined }}
                      ...
                      {orderedVisibleCols.map(key => {
                        ...
                        switch (key as ColKey) {
                          case 'imagen': return (
                            <td key={key} style={{ ...tdS, padding: '4px 6px' }}>
                              {it.imagen
                                ? <img src={it.imagen} alt="" style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 5, border: '1px solid var(--n-150)', cursor: 'pointer', display: 'block' }} onClick={() => window.open(it.imagen!, '_blank')} />
                                : <div style={{ width: 40, height: 40, borderRadius: 5, border: '1px dashed var(--n-200)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ImageIcon size={14} style={{ color: 'var(--n-300)' }} /></div>}
                            </td>
                          )
                          case 'proceso': return (
                            <td key={key} style={tdS}>
                          ...
                          case 'tipo':        return <td key={key} style={{ ...tdS, color: 'var(--n-600)', whiteSpace: 'nowrap' }}>{it.tipo}</td>
                          case 'cantidad':    return <td key={key} style={{ ...tdS, fontWeight: 600, whiteSpace: 'nowrap' }}>{it.cantidad}</td>
                          case 'material':    return <td key={key} style={{ ...tdS, color: 'var(--n-500)', whiteSpace: 'nowrap' }}>{it.material}</td>
                          case 'masa':        return <td key={key} style={{ ...tdS, color: 'var(--n-500)', whiteSpace: 'nowrap' }}>{it.masa}</td>
                          case 'codigo':      return <td key={key} style={{ ...tdS, color: 'var(--n-500)', whiteSpace: 'nowrap', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis' }} title={it.codigo}>{it.codigo || '—'}</td>
                          case 'comentarios': return (
                            <td key={key} style={{ ...tdS, color: 'var(--n-500)', fontSize: 11, minWidth: 160, maxWidth: 260 }}>
                              <div style={{ display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: 1.45, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                          ...
                          case 'ducasse':
                          case 'em_metal': {
                            const prov = PRICE_COL_MAP[key]
                            const val  = Number(it[prov.key]) || 0
                            const isBest = best && best.label === prov.label && val > 0
                            return (
                              <td key={key} style={{ ...tdS, textAlign: 'right', fontWeight: val > 0 ? 600 : 400, color: isBest ? 'var(--green-700)' : val > 0 ? 'var(--n-800)' : 'var(--n-300)', background: isBest ? 'var(--green-50)' : undefined }}>
                                {val > 0 ? `S/ ${val.toFixed(2)}` : '—'}
                              </td>
                            )
                          }
                          case 'mejor_precio': return (
                            <td key={key} style={{ ...tdS, textAlign: 'right' }}>
                              {best
                                ? <span style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--green-700)', background: 'var(--green-50)', padding: '2px 6px', borderRadius: 20, whiteSpace: 'nowrap' }}>{best.label}: S/ {best.val.toFixed(2)}</span>
                                : <span style={{ color: 'var(--n-300)' }}>—</span>}
                            </td>
                          )
                          case 'observaciones': return (
                            <td key={key} style={{ ...tdS, color: 'var(--n-500)', fontSize: 11, minWidth: 180, maxWidth: 300 }}>
                              <div style={{ display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: 1.45, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 4px', marginTop: 8 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            {pageNumbers(totalPages, page).map((p, i) =>
              p === '…'
                ? <span key={`e-${i}`} style={{ padding: '0 4px', fontSize: 12, color: 'var(--n-400)' }}>…</span>
                : <button key={p} onClick={() => setPage(p as number)} style={pageBtnS(false, p === page)}>{p}</button>
            )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}
          ...
          <div style={{ background: 'var(--n-0)', borderRadius: 12, padding: '20px 22px 18px', width: '100%', maxWidth: 620, maxHeight: '90vh', overflowY: 'auto', boxShadow: 'var(--shadow-lg)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
              <div><FL>#ITEM</FL><input value={form.item} onChange={e => setForm(p => ({ ...p, item: e.target.value }))} placeholder="Ej: 42" style={iStyle} /></div>
              <div><FL>Proceso</FL>
              ...
              <div style={{ gridColumn: '1/-1' }}>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <div style={{ flexShrink: 0, width: 72, height: 72, borderRadius: 8, border: '1px solid var(--n-200)', overflow: 'hidden', background: 'var(--n-50)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {previewUrl ? <img src={previewUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <ImageIcon size={20} style={{ color: 'var(--n-300)' }} />}
                  </div>
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}
                      ...
                      <Upload size={12} />{uploading ? 'Subiendo…' : previewUrl ? 'Cambiar imagen' : 'Subir imagen'}
                    </button>
                    {previewUrl && (
              ...
              <div style={{ gridColumn: '1/-1' }}><FL>Descripción *</FL><input value={form.descripcion} onChange={e => setForm(p => ({ ...p, descripcion: e.target.value }))} placeholder="Descripción del material o equipo" style={iStyle} autoFocus /></div>
              <div><FL>Tipo</FL><input value={form.tipo} onChange={e => setForm(p => ({ ...p, tipo: e.target.value }))} placeholder="Ej: Pernería, Corte" style={iStyle} /></div>
              <div><FL>Cantidad</FL><input value={form.cantidad} onChange={e => setForm(p => ({ ...p, cantidad: e.target.value }))} placeholder="Ej: 4, 1500 mm" style={iStyle} /></div>
              <div><FL>Material</FL><input value={form.material} onChange={e => setForm(p => ({ ...p, material: e.target.value }))} placeholder="ASTM A36, Zincado…" style={iStyle} /></div>
              <div><FL>Masa</FL><input value={form.masa} onChange={e => setForm(p => ({ ...p, masa: e.target.value }))} placeholder="Ej: 1.5 kg" style={iStyle} /></div>
              <div><FL>DXF</FL><input value={form.dxf} onChange={e => setForm(p => ({ ...p, dxf: e.target.value }))} placeholder="SI / NO / TUBO" style={iStyle} /></div>
              <div><FL>Código</FL><input value={form.codigo} onChange={e => setForm(p => ({ ...p, codigo: e.target.value }))} placeholder="Código de referencia" style={iStyle} /></div>
              <div style={{ gridColumn: '1/-1' }}><FL>Comentarios</FL>
              ...
              <div style={{ gridColumn: '1/-1' }}><FL>Observaciones</FL>
              ...
              {customCols.length > 0 && (
                <div style={{ gridColumn: '1/-1', borderTop: '1px solid var(--n-150)', paddingTop: 10, marginTop: 2 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--brand-500)', marginBottom: 8, letterSpacing: '0.04em' }}>CAMPOS PERSONALIZADOS</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                    {customCols.map(col => (
                      <div key={col.id}><FL>{col.label}</FL>
                        <input
                          type={col.type === 'number' ? 'number' : 'text'}
                          value={(form.custom_fields ?? {})[col.id] ?? ''}
                          ...
                          style={col.type === 'number' ? { ...iStyle, textAlign: 'right' } : iStyle}
                        />
                      </div>
                    ))}
                  </div>
...
const selectS: React.CSSProperties = { height: 30, padding: '0 8px', borderRadius: 7, border: '1px solid var(--n-200)', background: 'var(--n-0)', fontSize: 12, color: 'var(--n-700)', cursor: 'pointer' }
